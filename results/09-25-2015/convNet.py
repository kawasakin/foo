...
def gen_chip_feature(fname, numItems=11041):
    with open(fname) as fin:
        dat = np.loadtxt(StringIO(fin.read()), dtype="float32") 
    dat_X = dat.reshape(numItems, 1, 17, -1)
    return dat_X
...
def model(X, w1, w2, w3, w4, Max_Pooling_Shape, p_drop_conv, p_drop_hidden):
    ...
    l3 =  dropout(rectify(T.dot(l2, w3)), p_drop_hidden)
    pyx = softmax(T.dot(l3, w4))
    return pyx
...
dat_X = gen_chip_feature("datX.dat", dat_Y.shape[0])
...
w2 = init_weights((chip_motif_num, hidden_unit_num1))
w3 = init_weights((hidden_unit_num1, hidden_unit_num2))
w4 = init_weights((hidden_unit_num2, num_class))


noise_py_x = model(X, w1, w2, w3, w4, max_pool_shape, p_drop_conv, p_drop_hidden)
py_x = model(X, w1, w2, w3, w4, max_pool_shape, 0., 0.)
